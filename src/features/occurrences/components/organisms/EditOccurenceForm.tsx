...
export default function EditOccurrenceForm({
  ...
  if (isIllegalParking(occurrence))
    return <EditIllegalParkingForm occurrence={occurrence} />;
  return <EditTrafficAccidentForm occurrence={occurrence} />;
}

function EditWarningForm({
  occurrence,
}: {
  occurrence: Extract<Occurrence, { warningType: string }>;
}) {
        ...
        <form onSubmit={handleSubmit(() => setIsOpenConfirmation(true))}>
          <FieldGroup className="gap-4 mb-6">
            ...
            <FormField
              text="tipo do aviso"
              id="warningType"
              registration={register("warningType")}
...
function EditIllegalParkingForm({
  occurrence,
}: {
  occurrence: Extract<Occurrence, { parkingSpaceType: string }>;
}) {
  const router = useRouter();
  const [isOpenConfirmation, setIsOpenConfirmation] = useState(false);
        ...
        <form onSubmit={handleSubmit(() => setIsOpenConfirmation(true))}>
          <FieldGroup className="gap-4 mb-6">
            ...
            <FormField
              text="tipo de vaga"
              id="parkingSpaceType"
              registration={register("parkingSpaceType")}
              error={errors.parkingSpaceType}
            />
            <FormField
              text="descrição"
              id="description"
...
function EditTrafficAccidentForm({
  occurrence,
}: {
  occurrence: Extract<Occurrence, { trafficOccurrenceType: string }>;
}) {
  const router = useRouter();
  const [isOpenConfirmation, setIsOpenConfirmation] = useState(false);
